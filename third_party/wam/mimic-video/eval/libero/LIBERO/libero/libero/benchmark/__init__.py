...
class Benchmark(abc.ABC):
    ...
    def get_task_demonstration(self, i):
        assert 0 <= i and i < self.n_tasks, f"[error] task number {i} is outer of range {self.n_tasks}"
...
class LIBERO_90(Benchmark):
    def __init__(self, task_order_index=0):
        super().__init__(task_order_index=task_order_index)
        assert task_order_index == 0, "[error] currently only support task order for 10-task suites"
        self.name = "libero_90"
        self._make_benchmark()
